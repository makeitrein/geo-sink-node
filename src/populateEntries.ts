...
import { generateProposalId, generateTripleId, generateVersionId } from './utils/id';
import { pool } from './utils/pool';
import { Entry, ZodUriData, type FullEntry } from './zod';

export const populateEntries = async ({ entries, blockNumber, timestamp, cursor }: StreamData) => {
    const fullEntries: FullEntry[] = [];
    const uriResponses = await Promise.all(entries.map(entry => actionsFromURI(entry.uri)));
    ...
    const proposed_versions: s.proposed_versions.Insertable[] = toProposedVersions({
        fullEntries,
        blockNumber,
        timestamp,
        cursor,
    ...
    const triplesDatabaseTuples: TripleDatabaseTuple[] = toTripleDatabaseTuples(fullEntries);
    ...
    for (const [tupleType, triple] of triplesDatabaseTuples) {
        ...
        isCreateTriple
            ? await db.upsert('triples', triple, ['id'], { updateColumns: db.doNothing }).run(pool) // Protecting against duplicate triples that should have been deleted before creating again, but weren't
...
export const toActions = ({ fullEntries, cursor }: ToActionArgs) => {
    const actions: s.actions.Insertable[] = fullEntries.flatMap((fullEntry, entryIdx) => {
        return fullEntry.uriData.actions.map(action => {
            const string_value = action.value.type === 'string' ? action.value.value : null;
...
export const toGeoEntities = ({ fullEntries }: toGeoEntitiesArgs) => {
    ...
    fullEntries.forEach(fullEntry => {
        fullEntry.uriData.actions.map(action => {
            const { isNameCreateAction, isNameDeleteAction, isDescriptionCreateAction, isDescriptionDeleteAction } =
            ...
            const updatedName = isNameCreateAction ? action.value.value : isNameDeleteAction ? null : currentName;
...
export const toProposals = ({ fullEntries, blockNumber, timestamp, cursor }: toProposalsArgs) => {
    const proposals: s.proposals.Insertable[] = fullEntries.flatMap((fullEntry, entryIdx) => ({
...
export const toProposedVersions = ({ fullEntries, blockNumber, timestamp, cursor }: toProposedVersionArgs) => {
    const proposedVersions: s.proposed_versions.Insertable[] = fullEntries.flatMap((fullEntry, entryIdx) => {
...
export const toVersions = ({ fullEntries, blockNumber, timestamp, cursor }: toVersionArgs) => {
    const versions: s.versions.Insertable[] = fullEntries.flatMap((fullEntry, entryIdx) => {
        const uniqueEntityIds = fullEntry.uriData.actions
            .map(action => action.entityId)
            .filter((value, index, self) => self.indexOf(value) === index);
